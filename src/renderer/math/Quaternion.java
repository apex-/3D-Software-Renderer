	...
	//From Ken Shoemake's "Quaternion Calculus and Fast Animation" article
		public Quaternion(Matrix4f rot)
		{
			...
			if(trace > 0)
			{
				...
				m_w = 0.25f / s;
				m_x = (rot.m23 - rot.m32) * s;
				m_y = (rot.m31 - rot.m13) * s;
				m_z = (rot.m12 - rot.m21) * s;
			}
			else
			{
				if(rot.m11 > rot.m22 && rot.m11 > rot.m33)
				{
					float s = 2.0f * (float)Math.sqrt(1.0f + rot.m11 - rot.m22 - rot.m33);
					m_w = (rot.m21 - rot.m32) / s;
					m_x = 0.25f * s;
					m_y = (rot.m21 + rot.m12) / s;
					m_z = (rot.m31 + rot.m13) / s;
				}
				else if(rot.m22 > rot.m33)
				{
					float s = 2.0f * (float)Math.sqrt(1.0f + rot.m22 - rot.m11 - rot.m33);
					m_w = (rot.m31 - rot.m13) / s;
					m_x = (rot.m21 + rot.m12) / s;
					m_y = 0.25f * s;
					m_z = (rot.m32 + rot.m23) / s;
				}
				else
				{
					float s = 2.0f * (float)Math.sqrt(1.0f + rot.m33 - rot.m11 - rot.m22);
					m_w = (rot.m12 - rot.m21 ) / s;
					m_x = (rot.m31 + rot.m13 ) / s;
					m_y = (rot.m23 + rot.m32 ) / s;
					m_z = 0.25f * s;
				}
			}
